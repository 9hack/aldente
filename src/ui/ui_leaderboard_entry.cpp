...
UILeaderboardEntry::UILeaderboardEntry(float start_x, float start_y,
        ...
        : UIContainer(start_x, start_y),
          ...
          ranking_image(0.1f * height, 0.1f * height, // starting coordinates
                        0.8f * height, 0.8f * height,
                        AssetLoader::get_texture(ranking_to_image[ranking])),
          portrait_image(1.f * height, 0.1f * height,
                         0.8f * height, 0.8f * height,
                         portrait),
          gold_amount("0",
                      glm::max(1.8f * height, width - 1.2f * height),
                      0.25f * height,
                      0.8f * height, 0.5f * height,
                      Color::WHITE),
          gold_image(glm::max(1.8f * height, width - 1.2f * height) + 0.8f * height,
